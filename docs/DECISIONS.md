...
- **控制列最終形態:雙列合併盒 + ⚙ 於卷軸列,選單僅收主題** — `e8694c6`(歷程 `899fb6a` 曾把 A4/八度/滑動/音色全收選單)
  為何:動態島/瀏海機橫幅可用寬僅 ~714/738px,全部平鋪會爆三排。最終:「琴鍵數・八度」
  「音色・滑動」各併雙列盒、A4 留主列,⚙ 移卷軸列右端(不佔控制列寬),選單只收主題。
  實測一般(832)/瀏海(738)/動態島(714)皆兩排。使用者定案:常用功能要在主界面,
  只有「設定後暫時不動」的主題才收選單。
