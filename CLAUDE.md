...
## 技術棧(寫死)
- Vanilla JS + Tone.js(**自帶** `js/vendor/Tone.js` v14.8.49,禁 CDN)+ 純 CSS。**無框架、無 build step**。
- 目標:iOS Safari 15+(主)、桌機 Chrome/Safari(開發)。RWD、橫幅。
- 部署:GitHub Pages(靜態、HTTPS)。線上:https://marimbacheng.github.io/tunable-piano/
- **離線 PWA**:手寫 `sw.js`(無 Workbox/Vite,符合無建置)precache 全資源;首次連網後可完全離線。

## 目錄結構
```
index.html          結構(控制列/設定選單面板/卷軸列+⚙/鍵盤)、解鎖層、A2HS meta、自帶 Tone.js、SW 註冊
sw.js               離線 PWA Service Worker(precache 全資源、cache-first、版本化 CACHE)
manifest.json       PWA(加入主畫面 名稱=Tunable Piano)
icons/              程式產生的鋼琴 App 圖示(180/192/512)
js/vendor/Tone.js   自帶 Tone.js v14.8.49(離線需同源 precache,原走 CDN)
...
- **離線 PWA(SW)**:`sw.js` precache 清單須含**全部** 24 個 mp3(缺任一→離線切鋼琴=無聲);
  Tone.js 禁 CDN(跨源無法可靠 precache);**改任何資產務必 bump `sw.js` 的 `CACHE` 版本號**,否則舊 SW 永遠供舊快取。
